<?php
	...
	{
		// select Name from db to delete
		$stmt_select = $DB_con->prepare('SELECT id,name FROM address WHERE id =:uid');
		$stmt_select->execute(array(':uid'=>$_GET['delete_id']));
		$Row=$stmt_select->fetch(PDO::FETCH_ASSOC);
	
		
		// it will delete an actual record from db
		$stmt_delete = $DB_con->prepare('DELETE FROM address WHERE id =:uid');
...
<?php
	...
	$stmt = $DB_con->prepare('SELECT id,name, surname, mobile FROM address ORDER BY name DESC');
	$stmt->execute();
